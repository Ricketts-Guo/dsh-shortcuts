...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const inject = ['slots', 'sessions', 'remote', 'timer', 'uiSession', 'uiWorkspace'];
		...
		let currentSessionId = null; // 由 apply 中的当前会话订阅维护
		...
		function getState() { return { settings, paletteOpen, cheatsheetOpen, toast: toastMsg, currentSessionId }; }
		...
		function diagnosticInfo() {
			...
			try {
				...
				if (!permView) permState = '投影缺失';
					...
					permState = snap && Array.isArray(snap.options) ? '可用（' + snap.options.length + ' 档）'
						: snap && typeof snap.currentValue === 'string' ? '可用（' + snap.currentValue + '；目录按需读取）' : '无数据';
				}
		...
		const FEATURES = [
			...
			{ id: 'newSession', group: '会话', label: '新建会话', description: '开始一个新会话（沿用当前或最近的工作区）', defaultCombo: lead + '+N', run: () => {
				const svc = pluginCtx && pluginCtx.get('uiWorkspace');
			...
			{ id: 'stopTask', group: '会话', label: '停止当前任务', description: '中断正在运行的 agent 回合（等价于点击停止按钮）', defaultCombo: lead + '+.', run: () => {
				...
				try {
					const scoped = sessionsBinding(currentSessionId)?.ctx;
			...
			{ id: 'toggleDetails', group: '视图', label: '切换详情面板', description: '打开或关闭右侧详情栏', defaultCombo: lead + '+Shift+D', run: () => {
				...
				if (svc && currentSessionId) svc.toggleExpanded();
			} },
		...
		async function cyclePermissionRun() {
			...
			const binding = sessionsBinding(sessionId);
			const session = binding && binding.session;
			if (!session) {
			...
			const attempt = async (perm, retriesLeft) => {
				// alpha.2 separates the process-wide preset catalog from the
				// session projection. Earlier releases carry options in the projection.
				if (perm && !Array.isArray(perm.options)) {
					try {
						const api = pluginCtx && pluginCtx.get('remote.permissionPresets');
						const result = api && await api.catalog();
						if (result?.ok && Array.isArray(result.value?.options)) {
							perm = { ...perm, options: result.value.options };
						}
					} catch (err) { /* unavailable catalog follows the retry/error path */ }
				}
				...
				const applyPerm = (preset) => {
					try {
						const p = fetch('/dsh-shortcuts-permission?sessionId=' + encodeURIComponent(sessionId) + '&preset=' + encodeURIComponent(preset), { method: 'POST' });
		...
		function Palette() {
			const state = useStoreState();
			// Root overlay slots no longer pass useSessions implicitly.
			const store = pluginCtx.get('sessions').list;
			const list = React.useSyncExternalStore(store.subscribe, store.getSnapshot, store.getSnapshot);
		...
		function PaletteInner(props) {
			...
			if (list && Array.isArray(list.ids) && list.byId) {
				for (const id of list.ids) {
					...
					sessionRows.push({ id, title: sum.displayTitle, cwd: sum.cwd, running: !!sum.running, current: id === currentSessionId });
			...
			const workspacesSvc = pluginCtx && pluginCtx.get('uiWorkspace');
			...
			const choose = (i) => {
				...
				if (!row || !workspacesSvc) return;
				try { workspacesSvc.openSession(row.id); } catch (err) { /* 忽略未知会话 */ }
				setPaletteOpen(false);
			};
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const current = ctx.get('uiSession')?.current;
				const sync = () => {
					try {
						const snap = sessionsSvc.list.getSnapshot();
						const next = current?.getSnapshot ? current.getSnapshot()?.key : snap?.current;
						if (currentSessionId !== (next || null)) {
				...
				sync();
				const stopList = sessionsSvc.list.subscribe(sync);
				const stopCurrent = current?.subscribe?.(sync);
				return () => {
					stopList?.(); stopCurrent?.();
					currentSessionId = null; // Cancel any pending asynchronous permission lookup.
				};
