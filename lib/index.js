 ...
 * mounts the permission and connection services. It applies DSH's Host/Origin
 * and browser-authentication fence, accepts POST only, and validates the session id against the
...
export function apply(ctx) {
  ...
  ctx.inject(['webServer', 'permissionPresets', 'sessions', 'connection'], (hostCtx) => {
    hostCtx.effect(() => hostCtx.webServer.register({
      kind: 'prefix',
      path: '/dsh-shortcuts-permission',
      handler: (req, res) => {
        try {
          const rejection = hostCtx.connection.requestRejection(req);
          if (rejection !== undefined) {
            writeJson(res, rejection, { ok: false, error: rejection === 401 ? 'unauthorized' : 'forbidden' });
            return;
          }
          if (req.method !== 'POST') {
            writeJson(res, 405, { ok: false, error: 'POST required' }, { allow: 'POST' });
            return;
          }
