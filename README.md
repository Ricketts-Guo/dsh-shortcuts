...
- **权限 toast 三色反馈**：只读绿 / 工作区写入蓝 / 完全访问橙
...
| 模型 | 选择模型 1–9 `⌘1`–`⌘9` · 思考强度 1–5 `⌘⇧1`–`⌘⇧5` · 循环思考强度 |
...
- **无留痕**：不走 `/permission` 命令系统（命令执行会在对话流留下节点），而是通过宿主侧通道直调 `permissionPresets` 服务 —— 与命令处理函数同一服务，跳过审计日志，对话流保持干净。
- **三色反馈**：切换成功时底部 toast 按权限着色 —— 只读 🟢 绿、工作区写入 🔵 蓝、完全访问 🟠 橙。
- **服务端校验**：会话必须存在、预设必须在配置表中，非法请求返回明确错误。
...
- 静态安装：从 `~/.dsh/profiles/web/package.json` 移除两处 `dsh-shortcuts` 引用，删除 `node_modules/dsh-shortcuts` 链接，重启 DSH。
...
设置页、速查表、冲突检测、持久化、键盘分发全部由注册表自动派生 —— 新增功能只需加一行。组合键匹配做了上档字符归一化（`⌘⇧1` 按下时 `e.key` 是 `!`），录制与匹配自洽。
